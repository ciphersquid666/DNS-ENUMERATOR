...
void save_to_txt(const char *hostname, const char *ip_address, const char **directories, size_t dir_count) {
    ...
    fclose(file);
    print_color_text("Results saved to scan_results.txt\n", "\033[1;36m");
}
...
int main() {
    ...
    print_color_text("\nEnter the hostname to scan (type 'exit' to quit): ", "\033[1;36m");
    ...
    while (1) {
        ...
        if (strlen(ip_address) > 0) {
            ...
            print_color_text("The specified domain does not exist. Try again with a valid domain: ", "\033[1;31m");
        }
    }
